...
interface SavedTeamsMenuProps {
  ...
  /** When provided, the menu is fully controlled and renders no triggers. */
  saveOpen?: boolean;
  onSaveOpenChange?: (open: boolean) => void;
  loadOpen?: boolean;
  onLoadOpenChange?: (open: boolean) => void;
  /** Hide the built-in trigger buttons (use when triggers live elsewhere). */
  hideTriggers?: boolean;
}

export const SavedTeamsMenu = ({
  team,
  onLoad,
  saveOpen: controlledSaveOpen,
  onSaveOpenChange,
  loadOpen: controlledLoadOpen,
  onLoadOpenChange,
  hideTriggers = false,
}: SavedTeamsMenuProps) => {
  ...
  const saveOpen = controlledSaveOpen ?? internalSaveOpen;
  const setSaveOpen = onSaveOpenChange ?? setInternalSaveOpen;
  const loadOpen = controlledLoadOpen ?? internalLoadOpen;
  const setLoadOpen = onLoadOpenChange ?? setInternalLoadOpen;
  ...
  // The Load dropdown can be opened either via its built-in trigger button or
  // controlled externally. When triggers are hidden but the dropdown is open,
  // we anchor it to a hidden span at the document root so it still positions.
  const loadContent = (
  ...
  return (
    <>
      ...
      {/* When triggers are hidden, render a headless DropdownMenu so external
          controllers can still open the Load list. We anchor it to a tiny
          invisible span at the top-right of the viewport. */}
      {hideTriggers && (
        <DropdownMenu open={loadOpen} onOpenChange={setLoadOpen}>
          <DropdownMenuTrigger asChild>
            <span
              aria-hidden
              className="fixed top-2 right-2 h-0 w-0 pointer-events-none"
            />
          </DropdownMenuTrigger>
          {loadContent}
        </DropdownMenu>
